--[[
    Script: Sasiperere.lua (Versão Final com Silent Aim CORRIGIDO)
    [!] CORREÇÃO: A arma não atirava porque um erro no hook do Silent Aim impedia
    a função original de ser chamada. A lógica foi encapsulada em um `pcall`
    para garantir que, mesmo se o Silent Aim falhar, a arma sempre atire.
]]
...
local mainFrame = Instance.new("Frame", mainGui); mainFrame.Name = "DeleteMobF"; mainFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 25); mainFrame.Position = UDim2.new(0.5, -200, 0.5, -185); mainFrame.Size = UDim2.new(0, 400, 0, 370); mainFrame.Draggable = true; mainFrame.Active = true; mainFrame.Visible = true; mainFrame.ClipsDescendants = true
...
local function isVisible(origin, target, ...) local ignore = { Camera, ... }; if isAlive(LocalPlayer) then table.insert(ignore, LocalPlayer.Character) end; local ray = Ray.new(origin, target.Position - origin); local hit = workspace:FindPartOnRayWithIgnoreList(ray, ignore, false, true); return hit and hit:IsDescendantOf(target.Parent) end
local function getAi() local ai = {}; local aiZones = workspace:FindFirstChild("AiZones"); if not aiZones then return {} end; for _,v in aiZones:GetChildren() do for _, character in v:GetChildren() do table.insert(ai, character) end end; return ai end
local function getTarget(...)
    ...
    for _, player in Players:GetPlayers() do if player ~= LocalPlayer and isAlive(player) then if not (Settings.TeamCheck and player.Team == LocalPlayer.Team) then local hitPart = player.Character:FindFirstChild(Settings.HitPart); if hitPart then if not (Settings.WallCheck and not isVisible(Camera.CFrame.Position, hitPart, ...)) then local screenPosition, onScreen = Camera:WorldToViewportPoint(hitPart.Position); if onScreen then local distance = (Vector2.new(screenPosition.X, screenPosition.Y) - Camera.ViewportSize/2).Magnitude; if distance < closestDistance then closestDistance = distance; cloestTarget = hitPart end end end end end end end
    return cloestTarget
end
...
local function projectileDrop(origin, target, projectileSpeed, acceleration) local g = Vector3.yAxis * (acceleration * 2); local t = getBallisticFlightTime(target - origin, g, projectileSpeed); return 0.5 * g * t^2 end
local function predict(target, origin, projectileSpeed, acceleration) local g = Vector3.yAxis * (acceleration * 2); local t = getBallisticFlightTime(target.Position - origin, g, projectileSpeed); return target.Position + (target.Velocity * t) end
...
pcall(function()
    ...
    local oldBullet; oldBullet = hookfunction(bulletModule.CreateBullet, function(...)
        ...
        if not success then
            warn("ERRO NO HOOK DO SILENT AIM:", err)
        end
        -- [!] FIM DA REDE DE SEGURANÇA

        -- A arma SEMPRE atira, mesmo se o pcall falhar.
